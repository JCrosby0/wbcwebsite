<template>
  <main>
    ...
    <h1>This week's Thursday dinner</h1>
...
async function doRoutine(context) {
  ...
  const postID = pageFeed.data.find((p) => {
    return p.message.toLowerCase().contains('dinner')
  }).id
...
export default {
  ...
  computed: {
    ...
    postImages() {
      ...
      if (this.content.attachments.data[0].subattachments) {
